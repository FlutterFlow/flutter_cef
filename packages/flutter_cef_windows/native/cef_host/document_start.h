// Document-start scripts + JS channels, shared by the browser process and the
// renderer — both halves live in cef_host_win.cc on Windows (one exe re-runs as
// every sub-process).
//
// A copy of packages/flutter_cef_macos/native/cef_host/document_start.h, which
// is the reference: each federated package builds standalone, so the file is
// duplicated rather than shared. Keep the two identical below this header
// (test/document_start_parity_test.dart enforces it).
//
