...
#
# A probe reports with a `CEF_PROBE_RESULT PASS|FAIL` line on stdout (detail
# goes on `CEF_PROBE_LOG` / `CEF_PROBE_DETAIL` lines). This script kills only
# the processes it started, never other cef_host processes on the machine.
set -uo pipefail
cd "$(dirname "$0")/.."
ROOT="$PWD"
...
# name | entry point | timeout s | host | needs | note
#   host:   default (the host under test), false (/usr/bin/false), hang (never connects)
#   needs:  - ; shared (opens a named profile, so it runs with
#           FLUTTER_CEF_ALLOW_INSECURE_PROFILE=1: an ad-hoc host otherwise
#           downgrades the profile to ephemeral); or manual (interactive, a
...
PROBES=(
  ...
  "host_start_hang|host_start_failure_probe|60|hang|-|a host that never connects is disposed promptly"
  "keyboard_shortcut|keyboard_shortcut_probe|120|default|-|⌘-key combos reach the page before edit commands"
  "page_boundary|page_boundary_probe|180|default|-|popups, schemes and navigation stay inside the allowlist"
  ...
  "channel|channel_probe|60|default|-|page-to-app JS channel delivers"
  "channel_shared|channel_probe_shared|60|default|shared|two views on one host: each channel message reaches its own view"
  "multiview|multiview_probe|120|default|shared|agent control on several tiles of one shared host"
  "profile_reopen|profile_reopen_probe|180|default|shared|reopening a named profile never reports locked"
  ...
  "crispness|crispness_probe|0|default|manual|visual: HiDPI text crispness"
  "cull_wedge|cull_wedge_probe|0|default|manual|visual: cull and restore under load"
  "interaction_soak|interaction_soak_probe|0|default|manual|visual: input under load"
  "realsite_soak|realsite_soak_probe|0|default|manual|visual: real sites under load"
  "sharedhost_html|sharedhost_html_probe|0|default|manual|visual: authored HTML on a shared host"
  "zoom_soak|zoom_soak_probe|0|default|manual|visual: zoom churn"
)

if [ "${1:-}" = "--list" ]; then
  for e in "${PROBES[@]}"; do
    IFS='|' read -r name _ _ _ needs note <<< "$e"
...
echo "logs: $LOGS"
for e in "${PROBES[@]}"; do
  IFS='|' read -r name entry timeout host needs note <<< "$e"
  if [ $# -gt 0 ]; then
  ...
  if grep -q 'CEF_PROBE_RESULT PASS' "$log"; then
